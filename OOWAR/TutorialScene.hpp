...
namespace MyGame {
	...
	public:
			...
			, tutorialTextPos( 300,400 )
		{
			...
			{U"You can 'click' the green '→' to attack or move your ● to other one",m_playerTerritory.getPosition() + (m_playerTerritory.getPosition() - m_neutralTerritory1.getPosition()).normalized() * -200, tutorialTextPos},
			{U"one 'click' move only one ●.But you can use the Bottom to change the number.",Vec2(40, 460), tutorialTextPos},
